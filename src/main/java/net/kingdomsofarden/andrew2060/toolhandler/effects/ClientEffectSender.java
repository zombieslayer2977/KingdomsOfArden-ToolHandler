...
public class ClientEffectSender {
     ...
     * @param offset Where to play this effect in relation to the player's location
     ...
     */
    public static void playClientEffect(Player player, ClientEffectType effect, Vector offset, float speed, int count, boolean sendToAll) {
        if(!(player instanceof CraftPlayer)) {
            throw new IllegalArgumentException("The provided player is NOT a CraftPlayer!");
        }
        Location loc = player.getLocation();
        Packet63WorldParticles clientEffectPacket = new Packet63WorldParticles(effect.getParticleName(), (float)loc.getX(), (float)loc.getY(), (float)loc.getZ(), (float)offset.getX(), (float)offset.getY(), (float)offset.getZ(), speed, count);
        if(sendToAll) {
